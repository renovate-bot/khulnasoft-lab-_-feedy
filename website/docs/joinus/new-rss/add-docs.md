...
Now that we have completed the code, it's time to add the documentation for your route. Open the appropriate file in the [document (/website/docs)](https://github.com/khulnasoft-lab/feedy/blob/master/website/docs), which in this example is `website/docs/routes/programming.mdx`.
...
```tsx
<Route author="HenryQW" example="/github/issue/khulnasoft-lab/feedy" path="/github/issue/:user/:repo?" paramsDesc={['GitHub username', 'GitHub repo name, `FEEDy` by default']} />
```
...
```tsx
<Route author="khulnasoft-lab" example="/pixiv/search/Nezuko/popular/2" path="/pixiv/search/:keyword/:order?/:mode?" paramsDesc={['keyword', 'rank mode, empty or other for time order, `popular` for popular order, `date` by default', 'filte R18 content, see table below, empty by default']}>
...
```tsx
<Route author="HenryQW" example="/github/issue/khulnasoft-lab/feedy" path="/github/issue/:user/:repo?" paramsDesc={['GitHub username', 'GitHub repo name, `FEEDy` by default']} anticrawler="1" puppeteer="1" radar="1" configRequired="1" supportBT="1" supportPodcast="1" supportScihub="1" />
```
