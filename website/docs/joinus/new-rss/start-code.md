...
As mentioned earlier, we need to retrieve the GitHub username and repository name from user input. The repository name should default to `FEEDy` if it is not provided in the request URL. Here's how you can do it:
...
module.exports = async (ctx) => {
    // highlight-next-line
    const { user, repo = 'FEEDy' } = ctx.params;
...
module.exports = async (ctx) => {
    ...
    const repo = ctx.params.repo ?? 'FEEDy';
    // highlight-end
...
Both of these code snippets do the same thing. The first one uses object destructuring to assign the `user` and `repo` variables, while the second one uses traditional assignment with a nullish coalescing operator to assign the `repo` variable a default value of `FEEDy` if it is not provided in the request URL.
...
To do this, we can assign the relevant data to the `ctx.state.data` object, and FEEDy's middleware will take care of the rest.
...
The `parseDate` function is a utility function provided by FEEDy that we will use to parse dates later in the code.
...
As mentioned before, we want users to enter a GitHub username and a repository name, and fall back to `FEEDy` if they don't enter the repository name in the request URL.
...
In this code, `user` will be set to the value of `user` parameter, and `repo` will be set to the value of `repo` parameter if it exists, and `FEEDy` otherwise.
...
Assign them to the `ctx.state.data` object, and FEEDy's middleware will take care of the rest.
