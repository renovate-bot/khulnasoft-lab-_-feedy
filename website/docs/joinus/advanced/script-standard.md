...
When creating a new route in FEEDy, you need to organize your files in a specific way. Your namespace folder should be stored in the `lib/v2` directory and should include three mandatory files:

-   `router.js` Registers the routes
-   `maintainer.js` Provides information about the route maintainer
-   `radar.js` Provide a [FEEDy Radar](https://github.com/khulnasoft-lab/feedy-Radar) rule for each route
...
FEEDy appends the name of all route namespace folders in front of the actual route. Route maintainers should think of the namespace as the root.
...
Here's an example taken from the [furstar](https://github.com/khulnasoft-lab/feedy/blob/master/lib/v2/furstar) namespace:
