...
<Route author="khulnasoft-lab" example="/forum4399/mtag-83932" path="/forum4399/:mtag" paramsDesc={['mtag,必选-论坛网址最后的mtag字段']} configRequired="1">
  :::warning
...
<Route author="khulnasoft-lab" example="/coronavirus/scmp" path="/coronavirus/scmp" notOperational="1" />
...
<Route author="TonyRL" example="/google/alerts/FEEDy" path="/google/alerts/:keyword" paramsDesc={['Keyword']} />
...
<Route author="BrandNewLifeJackie26" example="/linkedin/jobs/C-P/1/software engineer" path="/linkedin/jobs/:job_types/:exp_levels/:keywords?" paramsDesc={['See the following table for details, use \'-\' as delimiter', 'See the following table for details, use \'-\' as delimiter', 'keywords']} radar="1">
  ...
  **To make it easier, the recommended way is to start a search on [LinkedIn](https://www.linkedin.com/jobs/search) and use [FEEDy Radar](https://github.com/khulnasoft-lab/feedy-Radar) to load the specific feed.**
</Route>
...
<Route author="bigfei" example="/linkedin/cn/jobs/Software" path="/linkedin/cn/jobs/:keywords?" paramsDesc={['搜索关键字']} radar="1" notOperational="1">
  ...
  **为了方便起见，建议您在 [LinkedIn.cn](https://www.linkedin.cn/incareer/jobs/search) 上进行搜索，并使用 [FEEDy Radar](https://github.com/khulnasoft-lab/feedy-Radar) 加载特定的 feed。**
</Route>

## MiniFlux {#miniflux}

因需设置 API Key，故请自行架设 FEEDy。API 密钥则请于 MiniFlux 实例中的 `设置` -> `API密钥` -> `创建一个新的API密钥` 处获取。
...
<Route author="emdoe" example="/miniflux/feeds=1&2&3/mark=read&limit=7&status=unread" path="/miniflux/:feeds/:parameters?" paramsDesc={['订阅源 ID 或获取全部', '过滤及设置参数，多个请用 `&` 连接']} configRequired="1">
  ...
  3.  可进一步根据需要，指定参数：
      ...
      3.  此外，本路由另支持额外的选项，包括：
          ...
          2.  利用 `mark` 参数控制在 FEEDy 获取订阅内容之后的动作，包括维持不变（`unchanged`，默认），标记为已读（`read`），移除（`removed`），以及标记为未读（`unread`）。**注意，请不要将标记为已读简单的理解为是用于实现某种同步服务的**，这个功能更像是在辅助 MiniFlux 的自动清理功能。
          ...
          4.  可以通过 `limit` 参数控制输出内容的数量，默认输出全部符合其它条件的所有内容，但**建议用户设置该参数**。不过众所周知，FEEDy 提供全局参数 `?limit` 控制输出内容的数量，但如果用户存在设置错误，则可能出现一些奇怪的问题：不妨考虑用户的参数设定为 `miniflux/all/limit=5&status=unread&mark=read?limit=3`，那么则有两篇用户未读的文章自动被标记为已读，用户却很可能从未发现。为避免出现这样的情形，本路由对该参数的设定作出如下调整：
...
<Route author="khulnasoft-lab" example="/dida365/habit/checkins" path="/dida365/habit/checkins" configRequired="1" />
