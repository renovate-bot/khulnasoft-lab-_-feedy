...
举例 1: [https://rsshub.app/bilibili/user/coin/2267573?filter=diyGOD|FEEDy&filter_case_sensitive=false](https://rsshub.app/bilibili/user/coin/2267573?filter=diyGOD|FEEDy&filter_case_sensitive=false)
...
FEEDy 同时支持 RSS 2.0、Atom、JSON Feed 和 RSS3 UMS 输出格式，在路由末尾添加 `.rss`、`.atom`、`.json` 或 `.ums` 即可请求对应输出格式，缺省为 RSS 2.0
...
在路由末尾添加 `.debug.json`且实例运行在`debugInfo=true`的情况下，FEEDy 将会返回插件设置在`ctx.state.json`的内容
...
在路由末尾添加 `.{index}.debug.html` （`{index}` 为数字，为从 0 开始的下标）且实例运行在 `debugInfo=true` 的情况下，FEEDy 将会返回插件设置在 `ctx.state.data.item[index].description` 的内容，你可用浏览器访问该页面来快速查看提取的信息的展示结果。
