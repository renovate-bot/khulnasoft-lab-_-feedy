...
部署 FEEDy 需要基本的计算机编程常识，如果您在部署过程中遇到无法解决的问题请到 [issues](https://github.com/khulnasoft-lab/feedy/issues) 寻找类似的问题或 [向我们提问](https://github.com/khulnasoft-lab/feedy/issues/new/choose)，我们会尽快给您答复。
...
默认推荐使用 `khulnasoft-lab/feedy` 即 `khulnasoft-lab/feedy:latest` 最新版镜像以获取最新路由。

当 `khulnasoft-lab/feedy:latest` 存在问题时，可以使用以日期为标签的近期镜像临时使用，例如：

```bash
$ docker pull khulnasoft-lab/feedy:2021-06-18
```

待最新镜像更新后再切换回 `khulnasoft-lab/feedy:latest` 最新版镜像。

如需启用 puppeteer，可使用 `khulnasoft-lab/feedy:chromium-bundled`；若指定日期则为 `khulnasoft-lab/feedy:chromium-bundled-2021-06-18`。

亦可使用 Docker Compose 部署以启用 puppeteer，但更消耗磁盘空间和内存。通过修改 `docker-compose.yml`，也可以使用 `khulnasoft-lab/feedy:chromium-bundled`，这样就没有更消耗资源的问题了。

## Docker Compose 部署

### 安装

下载 [docker-compose.yml](https://github.com/khulnasoft-lab/feedy/blob/master/docker-compose.yml)
...
修改 [docker-compose.yml](https://github.com/khulnasoft-lab/feedy/blob/master/docker-compose.yml) 中的 `environment` 进行配置

## Docker 部署

:::tip

如需启用 puppeteer，请在**每条**命令中均将 `khulnasoft-lab/feedy` 替换为 `khulnasoft-lab/feedy:chromium-bundled`。
...
配置运行在 docker 中的 FEEDy，最便利的方法是使用 docker 环境变量
...
该部署方式不包括 puppeteer（除非改用 `khulnasoft-lab/feedy:chromium-bundled`）和 redis 依赖，如有需要请改用 Docker Compose 部署方式或自行部署外部依赖

更多配置项请看 [#配置](/zh/install/config)

## Kubernetes 部署 (Helm)

FEEDy 可以使用来自 [FEEDy Helm Chart](https://github.com/NaturalSelectionLabs/helm-charts/tree/main/charts/rsshub) 的 Helm Chart 在 Kubernetes 中安装
...
要配置更多值，请参阅 [FEEDy Helm Chart](https://github.com/NaturalSelectionLabs/helm-charts/tree/main/charts/rsshub)。

## Ansible 部署

这个 Ansible playbook 包括了 FEEDy, Redis, browserless （依赖 Docker） 以及 Caddy 2
...
# 举例：如果您的 FEEDy 用户使用 https://rsshub.example.com 访问您的 FEEDy 实例，输入 rsshub.example.com（去掉 https://）
```
...
部署 `FEEDy` 最直接的方式，您可以按照以下步骤将 `FEEDy` 部署在您的电脑、服务器或者其他任何地方
...
在 `FEEDy` 文件夹中运行下面的命令就从 github 仓库拉取最新版本
...
1.  将 FEEDy [分叉（fork）](https://github.com/khulnasoft-lab/feedy/fork) 到自己的账户下。
2.  把自己的分叉部署到 Heroku：`https://heroku.com/deploy?template=URL`，其中 `URL` 改为分叉地址 （例如 `https://github.com/USERNAME/FEEDy`）。
3.  检查 Heroku 设置，随代码库更新自动部署。
4.  安装 [Pull](https://github.com/apps/pull) 应用，定期将 FEEDy 改动自动同步至你的分叉。
...
1.  将 FEEDy [分叉（fork）](https://github.com/khulnasoft-lab/feedy/fork) 到自己的账户下
2.  去 Vercel 部署一个新项目：使用 GitHub 账户登录 Vercel，进入 [项目创建页面](https://vercel.com/new/) 选择导入 FEEDy 仓库进行部署
3.  安装 [Pull](https://github.com/apps/pull) 应用，定期将 FEEDy 改动自动同步至你的仓库

## 部署到 Fly.io

### 方案一：Fork

1.  将 FEEDy [Fork](https://github.com/khulnasoft-lab/feedy/fork) 到自己的账户下；
...
来创建一个新的 Redis 数据库，地域选择与你上面创建 FEEDy app 时相同的地域，建议选择开启 [eviction](https://redis.io/docs/reference/eviction/)。创建完成后会输出类似于 `redis://default:<password>@<domain>.upstash.io` 的字符串。
...
每月只需 1 美元即可运行 FEEDy。包括自动更新和 5 美元的免费起始额度。
...
运行 `git clone https://github.com/khulnasoft-lab/feedy.git` 拉取本项目的最新版本。

### app.yaml 配置

#### 部署到 Flexible Environment

在 FEEDy 项目根目录下建立一个 app.yaml 文件，内容示例如下：
...
如果想要测试因为反爬规则导致无法访问的路由，您可以点击下方按钮拉起一套免费，临时，专属于您的 FEEDy
