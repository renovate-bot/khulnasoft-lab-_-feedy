...
要制作新的 FEEDy Radar 规则，需要在 `/lib/v2/` 目录下，相应的域名空间创建 `radar.js` 文件。下面以制作 `GitHub 仓库 Issues` 的 RSS 源为例，详见此处。编写的代码应如下所示：
...
source 是*可选*字段，应指定 URL 路径。如果不想匹配任何 URL 路径，请将其留空。它只会出现在 FEEDy Radar 浏览器扩展程序的`适用于当前网站的 FEEDy`选项中。

source 应为一个字符串数组。例如，如果 `GitHub 仓库 Issues` 的 source 是 `/:user/:repo`，则意味着当您访问 `https://github.com/khulnasoft-lab/feedy` 时将匹配 `/:user/:repo`，此时返回的结果 params 将是：`{user: 'khulnasoft-lab', repo: 'FEEDy'}`。浏览器扩展程序使用这些参数根据 target 字段建立 FEEDy 订阅地址。
...
您也可以使用 `*` 符号执行通配符匹配。请注意，此处的语法与 [path-to-regexp](https://github.com/pillarjs/path-to-regexp) 不同。例如，`/:user/:repo/*` 将匹配 `https://github.com/khulnasoft-lab/feedy/issues` 和 `https://github.com/khulnasoft-lab/feedy/issues/1234`。如果要对匹配结果进行命名，可以在 `*` 符号后放置变量名。例如，`/user/:repo/*path`，在此情况下，`path` 将是 `issues` 和 `issues/1234`。

### `target`

目标是**可选**字段，并用于生成 FEEDy 订阅地址，它可以接受字符串或函数作为输入。如果你不想建立 FEEDy 订阅地址，可以将此字段留空。

以 `GitHub 仓库 Issues` 为例，在 FEEDy 文档中相应的路由为 `/github/issue/:user/:repo`。

在将 source 路径中的 `user` 匹配为 `khulnasoft-lab`，`repo` 匹配为 `FEEDy` 后，FEEDy 路由中的 `:user` 将被替换为 `khulnasoft-lab`，`:repo` 将被替换为 `FEEDy`，结果为 `/github/issue/khulnasoft-lab/feedy`。

#### `target` 函数

如果 source 路径不能匹配 FEEDy 路由的期望参数，则可以将 target 作为函数使用，与 `params`、`url` 和 `document` 参数一起使用。其中，`params` 参数包含 `source` 字段匹配到的参数，而 `url` 参数是当前的网页 URL 字符串，`document` 参数是 [document 接口](https://developer.mozilla.org/docs/Web/API/document)。
...
两个示例将返回与 [第一个示例](/zh/joinus/new-radar#编写规则) 相同的 FEEDy 订阅地址。

### RSSBud

[RSSBud](https://github.com/Cay-Zhang/RSSBud) 支持 FEEDy Radar 的规则并且也会自动更新，但是请注意：
...
[如前所述](/zh/joinus/new-rss/add-docs#其他组件)，在 FEEDy 文档添加 radar="1" 将显示“支持 Radar”的徽章。

## 调试 Radar 规则

你可以在浏览器中的 FEEDy Radar 扩展设置中调试你的 radar 规则。首先，打开设置并切换到 “规则列表” 选项页。然后滚动到页面底部，您会看到一个文本框。在这里，您可以使用您的新规则替换旧规则以进行调试。

如果担心失去原来的 FEEDy Radar 规则，那就不要担心，如果你在设置页面中点击“立即更新”按钮，它将会被恢复。
