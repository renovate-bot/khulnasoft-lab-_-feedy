...
开始之前，您需要安装 FEEDy 的依赖项。您可以在 FEEDy 的根目录下运行以下命令来完成安装：
...
一旦您成功安装了依赖，您可以通过运行以下命令来开始调试 FEEDy：
...
请务必密切关注控制台输出的任何错误消息或其他有用的信息，这些信息可以帮助您诊断和解决问题。另外，如果您遇到任何困难，不要犹豫向 FEEDy 文档或社区寻求帮助。
...
制作新的 RSS 路由的第一步是创建命名空间。命名空间应该与您制作 RSS 源的主要网站的二级域名**相同**。例如，如果您正在为 [https://github.com/khulnasoft-lab/feedy/issues](https://github.com/khulnasoft-lab/feedy/issues) 制作 RSS 源，第二级域名是 `github`。因此，您应该在 `lib/v2` 下创建名为 `github` 的文件夹，作为您的 RSS 路由的命名空间。
...
例如，如果您为 [GitHub 仓库 Issues](/zh/routes/programming#github-yong-hu-cang-ku) 制作 RSS 源，并且假设您希望用户输入 GitHub 用户名和仓库名，如果他们没有输入仓库名，则返回到 `FEEDy`，您可以使用以下代码在 `github/router.js` 中注册您的新 RSS 路由：
...
在上面的示例中，`issue` 是一个精确匹配，`:user` 是一个必需参数，`:repo?` 是一个可选参数。`?` 在 `:repo` 之后表示该参数是可选的。如果用户没有输入仓库名，则会返回到您代码中指定的内容（这里是 `FEEDy`）。

一旦您定义了路由路径，您可以从 `ctx.params` 对象中获取参数的值。例如，如果用户访问了 `/github/issue/khulnasoft-lab/feedy`，您可以分别从 `ctx.params.user` 和 `ctx.params.repo` 中获取 `user` 和 `repo` 的值。例如，如果用户访问了 `/github/issue/khulnasoft-lab/feedy`，则 `ctx.params.user` 和 `ctx.params.repo` 将分别为 `khulnasoft-lab` 和 `FEEDy`。
...
文件可以帮助用户在使用 [FEEDy Radar](https://github.com/khulnasoft-lab/feedy-Radar) 或其他兼容其格式的软件时订阅您的新 RSS 路由。我们将在后面的部分更多介绍。
