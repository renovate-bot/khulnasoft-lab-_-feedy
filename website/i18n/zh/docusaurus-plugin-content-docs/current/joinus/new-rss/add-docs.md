...
现在我们完成了代码，是时候为您的路由添加文档了。在 [文档 (/website/docs)](https://github.com/khulnasoft-lab/feedy/blob/master/website/docs) 中打开相应的文件，本例中是 `website/docs/routes/programming.mdx`。
...
```tsx
<Route author="HenryQW" example="/github/issue/khulnasoft-lab/feedy" path="/github/issue/:user/:repo?" paramsDesc={['GitHub 用户名', 'GitHub 仓库名称，默认为 `FEEDy`']} />
```
...
```tsx
<Route author="khulnasoft-lab" example="/pixiv/search/Nezuko/popular/2" path="/pixiv/search/:keyword/:order?/:mode?" paramsDesc={['关键词', '排序方式，`popular` 按热门度排序，空或其他任意值按时间排，默认为 `date`', '过滤方式，见下表 默认为空']}>
...
```tsx
<Route author="HenryQW" example="/github/issue/khulnasoft-lab/feedy" path="/github/issue/:user/:repo?" paramsDesc={['GitHub 用户名', 'GitHub 仓库名称，默认为 `FEEDy`']} anticrawler="1" puppeteer="1" radar="1" configRequired="1" supportBT="1" supportPodcast="1" supportScihub="1" />
```
...
-   **别忘了关闭标签！**
-   在提交 Pull Request 之前，请务必运行在 FEEDy 的根目录运行以下命令检查和格式化您的代码：
