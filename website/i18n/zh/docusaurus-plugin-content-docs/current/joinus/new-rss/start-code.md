...
如前所述，我们需要从用户输入中获取 GitHub 用户名和仓库名称。如果请求 URL 中未提供仓库名称，则应默认为 `FEEDy`。您可以使用以下代码实现：
...
这两个代码片段都执行相同的操作。第一个使用对象解构将 `user` 和 `repo` 变量赋值，而第二个使用传统赋值和空值合并运算符在请求 URL 中未提供它的情况下将 `repo` 变量分配默认值 `FEEDy`。
...
为此，我们可以将相关数据赋值给 `ctx.state.data` 对象，FEEDy 的中间件将处理其余部分。
...
`parseDate` 函数是 FEEDy 提供的一个工具函数，在代码的后面我们会用到它来解析日期。
...
在这段代码中，`user` 将被设置为 `user` 参数的值，如果存在 `repo` 参数，则 `repo` 将被设置为该参数的值，否则为 `FEEDy`。
