...
当在 FEEDy 中编写新的路由时，需要按特定方式组织文件。命名空间文件夹应该存储在 `lib/v2` 目录下，并且应包括三个必需文件：

-   `router.js` 注册路由
-   `maintainer.js` 提供路由维护者信息
-   `radar.js` 为每个路由提供对应 [FEEDy Radar](https://github.com/khulnasoft-lab/feedy-Radar) 规则
...
FEEDy 会将所有路由命名空间的文件夹名附加到路由前面。路由维护者可命名空间视为根目录。
...
下面是在 [furstar](https://github.com/khulnasoft-lab/feedy/blob/master/lib/v2/furstar) 命名空间中示例：
