...
本指南面向希望深入了解如何制作 RSS 订阅源的高级用户。如果您是第一次制作 RSS 订阅源，我们建议先阅读 [制作自己的 FEEDy 路由](/zh/joinus/new-rss/start-code)。

一旦您获取了要包含在您的 RSS 订阅源中的数据，就可以将其传递给 `ctx.state.data`。然后FEEDy的中间件 [`template.js`](https://github.com/khulnasoft-lab/feedy/blob/master/lib/middleware/template.js) 将处理数据并以所需的格式呈现 RSS 输出（默认为RSS 2.0）。除了 [制作自己的 FEEDy 路由](/zh/joinus/new-rss/start-code) 中提到的字段外，您还可以使用以下字段进一步自定义 RSS 订阅源。

需要注意的是，并非所有字段都适用于所有的输出格式，因为 FEEDy 支持多种输出格式。下表显示了不同输出格式兼容的字段。我们使用以下符号表示兼容性：`A` 表示 Atom，`J` 表示 JSON Feed，`R` 表示 RSS 2.0。
...
| 字段            | 描述                  | 默认值 | 兼容性 |
| :----------      | :----------         | :----------- | :------------ |
| **`title`**       | *（推荐）* 源的名称，应为纯文本   | `FEEDy`     | A, J, R |
...
| **`author`**      | *（可选）* Atom 源的作者或 JSON Feed 的作者 | `FEEDy`     | A, J |
...
FEEDy 支持制作 BitTorrent／磁力订阅源，这将帮助你的 RSS 订阅源。要制作 BitTorrent／磁力订阅源，您需要在 RSS 源添加附加字段，以符合 BitTorrent 客户端的订阅格式。
...
如果您要在 FEEDy 路由中添加对 BitTorrent／磁力订阅支持，最重要的是在文档以反映此功能。要做到这一点，您需要将 `Route` 组件的 `supportBT` 属性设置为 `"1"`。 以下是一个示例：
...
FEEDy支持制作期刊订阅源。如果用户提供 [通用参数](/zh/parameter#输出-sci-hub-链接) `scihub`，则可以将 `item.link` 替换为 Sci-hub 链接。要制作期刊订阅源，您需要在您的 RSS 源中包含一个附加字段：
...
通过在 RSS 源中包含 `doi` 字段，您将能够制作与 FEEDy 的 Sci-hub 功能兼容的期刊订阅源。
...
FEEDy 支持制作与播客播放器订阅格式兼容的播客订阅源。要制作播客订阅源，您需要在RSS源中包含几个附加字段：
...
FEEDy支持制作与 [Media RSS](https://www.rssboard.org/media-rss) 格式兼容的媒体订阅源。要制作体订阅源订阅源，您需要在 RSS 源中包含这些附加字段。
...
FEEDy支持制作包含互动，如点赞、反对和评论的 Atom 订阅源。要制作带有互动的 Atom 订阅源，您需要在 RSS 源中包含附加字段，用于指定每个条目的互动计数。
