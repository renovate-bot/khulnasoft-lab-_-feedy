...
module.exports = async (ctx) => {
    ...
    let err; // !!! let FEEDy throw an anti-crawler prompt if the route is empty !!!
        ...
        const realLink = await ctx.cache.tryGet(`data258:${cacheId}`, async () => {
            try {
                ...
                if (response.data.includes('今日浏览次数已达上限')) {
                    ...
                    // !!! so that's not a big problem if the FEEDy instance is self-hosted !!!
    ...
    if (items.length === 0 && err) {
        ...
        // !!! let FEEDy throw an anti-crawler prompt !!!
        throw err;
    }
