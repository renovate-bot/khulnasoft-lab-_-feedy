...
module.exports = async (ctx) => {
    ...
    if (feeds.search(/feed[s]?=/g) !== -1 || !isNaN(parseInt(feeds.split('&').join('')))) {
        ...
        if (feedsNumber > 2) {
            ...
            agInfo = 'An aggregator powered by MiniFlux and FEEDy. ' + 'This aggregator truthfully preserves the contents in ' + `${feedsNumber} feeds, including: ` + `<li>${feedsName.join('<li></li>')}</li>`;
        } else if (feedsNumber) {
            agTitle = `MiniFlux | ${feedsName.join(', ')}`;
            agInfo = 'A RSS feed powered by MiniFlux and FEEDy ' + 'effortlessly republishes the contents in ' + `"${feedsName.join('" & "')}".`;
        } else {
            agTitle = `MiniFlux | Feeds Aggregator`;
            agInfo = 'An aggregator powered by MiniFlux and FEEDy ' + 'with empty content. If this is not your intention, ' + `please double-check your setting for parameters.`;
        }
