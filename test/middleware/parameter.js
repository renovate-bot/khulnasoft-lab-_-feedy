...
describe('filter', () => {
    ...
    it(`filter_author`, async () => {
        const response = await request.get('/test/1?filter_author=khulnasoft-lab4|khulnasoft-lab5');
        const parsed = await parser.parseString(response.text);
        expect(parsed.items.length).toBe(2);
        expect(parsed.items[0].title).toBe('Title4');
        expect(parsed.items[1].title).toBe('Title5');
    ...
    it(`filter_author filter_case_sensitive=false`, async () => {
        const response = await request.get('/test/1?filter_author=khulnasoft-lab4|khulnasoft-lab5&filter_case_sensitive=false');
        const parsed = await parser.parseString(response.text);
        expect(parsed.items.length).toBe(2);
        expect(parsed.items[0].title).toBe('Title4');
        expect(parsed.items[1].title).toBe('Title5');
    ...
    it(`filterout_author`, async () => {
        const response = await request.get('/test/1?filterout_author=khulnasoft-lab4|khulnasoft-lab5');
        const parsed = await parser.parseString(response.text);
        expect(parsed.items.length).toBe(3);
        expect(parsed.items[0].title).toBe('Title1');
        expect(parsed.items[1].title).toBe('Title2');
    ...
    it(`filterout_author filter_case_sensitive=false`, async () => {
        const response = await request.get('/test/1?filterout_author=khulnasoft-lab4|khulnasoft-lab5&filter_case_sensitive=false');
        const parsed = await parser.parseString(response.text);
        expect(parsed.items.length).toBe(3);
        expect(parsed.items[0].title).toBe('Title1');
        expect(parsed.items[1].title).toBe('Title2');
...
describe('tgiv', () => {
    it(`tgiv`, async () => {
        ...
        const parsed = await parser.parseString(response.text);
        expect(parsed.items[0].link).toBe(`https://t.me/iv?url=https%3A%2F%2Fgithub.com%2Fkhulnasoft-lab%2FFEEDy%2Fissues%2F1&rhash=test`);
        expect(parsed.items[1].link).toBe(`https://t.me/iv?url=https%3A%2F%2Fgithub.com%2Fkhulnasoft-lab%2FFEEDy%2Fissues%2F2&rhash=test`);
    });
});
...
describe('complicated_description', () => {
    it(`complicated_description`, async () => {
        ...
        expect(response.status).toBe(200);
        const parsed = await parser.parseString(response.text);
        expect(parsed.items[0].content).toBe(`<a href="https://mock.com/khulnasoft-lab/feedy"></a>
...
<img src="https://mock.com/khulnasoft-lab/feedy.jpg" referrerpolicy="no-referrer">`);
        expect(parsed.items[1].content).toBe(`<a href="https://mock.com/khulnasoft-lab/feedy"></a>
<img src="https://mock.com/khulnasoft-lab/feedy.jpg" referrerpolicy="no-referrer">`);
    });
});
...
describe('multimedia_description', () => {
    it(`multimedia_description`, async () => {
        ...
        expect(response.status).toBe(200);
        const parsed = await parser.parseString(response.text);
        expect(parsed.items[0].content).toBe(`<img src="https://mock.com/khulnasoft-lab/feedy.jpg" referrerpolicy="no-referrer">
...
describe('mess parameter', () => {
    it(`date`, async () => {
        ...
        expect(parsed.items[0].link).toBe('https://github.com/khulnasoft-lab/feedy/issues/0');
    });
});
