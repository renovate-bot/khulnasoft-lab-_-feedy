...
describe('template', () => {
    ...
    it('flatten author object', async () => {
        ...
        const parsed = await parser.parseString(response.text);
        expect(parsed.items[2].author).toBe(['khulnasoft-lab1', 'khulnasoft-lab2'].map((name) => name).join(', '));
        expect(parsed.items[3].author).toBe(['khulnasoft-lab3', 'khulnasoft-lab4', 'khulnasoft-lab5'].map((name) => name).join(', '));
    ...
    it(`enclosure`, async () => {
        ...
        const parsed = await parser.parseString(response.text);
        expect(parsed.itunes.author).toBe('khulnasoft-lab');
        expect(parsed.items[0].enclosure.url).toBe('https://github.com/khulnasoft-lab/feedy/issues/1');
